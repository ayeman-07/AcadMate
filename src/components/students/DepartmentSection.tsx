...
import SemesterSkeletonCard from "@/components/ui/SemesterSkeletonCard";
...
const DepartmentSection: FC<DepartmentSectionProps> = ({
    ...
    const onAddSemester = async (name: string, department: string) => {
        try {
            ...
            const newSem: Semester = { name, department, studentCount: 0 };

            setBatches((prev) => {
                const updatedDept = [...(prev[title] || []), newSem];
                const updatedAll = [...(prev["ALL"] || []), newSem];
